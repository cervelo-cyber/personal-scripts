...
# -----------------------
# 低内存环境下 apk update 易被 OOM 杀死:重试 + 清缓存 + 仅更新 main 索引
apk_update_retry() {
    local i repos repos_bak
    repos="/etc/apk/repositories"
    repos_bak="/etc/apk/repositories.apk-bak"

    # 低内存时临时只保留 main 仓库,索引内存减半(依赖均在 main)
    if [ -f "$repos" ]; then
        cp -f "$repos" "$repos_bak" 2>/dev/null || true
        if grep -v '^#' "$repos" | grep -v 'community\|testing' | grep -q 'main'; then
            grep -v '^#' "$repos" | grep -v 'community\|testing' > "${repos}.lowmem" 2>/dev/null
            mv -f "${repos}.lowmem" "$repos" 2>/dev/null || true
        fi
    fi
    ...
    for i in 1 2 3 4 5; do
        ...
        if apk update; then
            [ -f "$repos_bak" ] && mv -f "$repos_bak" "$repos" 2>/dev/null || true
            return 0
        fi
        warn "apk update 第 $i 次失败(可能内存不足被 OOM),清理缓存后重试..."
        sleep 2
    done
...
install_deps() {
    ...
    case "$OS" in
        alpine)
            apk_update_retry || { err "apk update 失败"; exit 1; }
...
# -----------------------
# 低内存下 apk 不可用时的二进制安装(绕过 apk/tmpfs,流式下载并解压到磁盘路径)
install_singbox_binary() {
    local arch ver tarball_url sb_tmp bin
    ...
    ver=$(curl -fsSL --max-time 30 "https://api.github.com/repos/SagerNet/sing-box/releases/latest" 2>/dev/null \
        | sed -n 's/.*"tag_name":[[:space:]]*"v\([^"]*\)".*/\1/p' | head -1)
    if [ -z "$ver" ]; then
        ver=$(curl -fsSL --max-time 30 "https://sing-box.app/install.sh" 2>/dev/null \
            | grep -oE 'sing-box-[0-9]+\.[0-9]+\.[0-9]+' | head -1 | sed 's/sing-box-//')
    fi
    if [ -z "$ver" ]; then
        err "无法获取 sing-box 最新版本号"
        return 1
    fi

    tarball_url="https://github.com/SagerNet/sing-box/releases/download/v${ver}/sing-box-${ver}-linux-${arch}.tar.gz"

    # 关键:低内存容器 /tmp 可能是 tmpfs(占用 RAM 不可回收),必须用磁盘路径并流式解压,不落整包
    sb_tmp="/usr/local/.sb-install-$$"
    rm -rf /var/cache/apk/* 2>/dev/null || true
    mkdir -p "$sb_tmp" || { err "无法创建临时目录"; return 1; }
    info "下载并解压 sing-box ${ver} (流式, 不经过 apk/tmpfs)..."
    if ! curl -fsSL --max-time 600 "$tarball_url" | tar -xz -C "$sb_tmp"; then
        rm -rf "$sb_tmp"
        err "下载/解压失败: $tarball_url"
        return 1
    fi
    bin=$(find "$sb_tmp" -type f -name sing-box | head -1)
    if [ -z "$bin" ]; then
        rm -rf "$sb_tmp"
        err "压缩包内未找到 sing-box 二进制"
        return 1
    fi
    install -m 755 "$bin" /usr/local/bin/sing-box || { rm -rf "$sb_tmp"; err "二进制安装失败"; return 1; }
    rm -rf "$sb_tmp"
    sync

    if [ -x /usr/local/bin/sing-box ] && [ ! -e /usr/bin/sing-box ]; then
        ln -s /usr/local/bin/sing-box /usr/bin/sing-box
    fi
    info "sing-box ${ver} 已通过二进制方式安装到 /usr/local/bin"
    return 0
}
...
install_singbox() {
    ...
    case "$OS" in
        alpine)
            ...
            APK_OK=""
            if command -v apk >/dev/null 2>&1 && [ "$(mem_total_mb)" -ge 96 ]; then
                if apk_update_retry >/dev/null 2>&1 && apk add --no-cache \
                    --repository=http://dl-cdn.alpinelinux.org/alpine/edge/community \
                    sing-box >/dev/null 2>&1; then
                    APK_OK=1
                    info "sing-box 已通过 apk 安装"
                else
                    warn "apk 安装失败(低内存环境下 apk update 易被 OOM 杀死),改用官方二进制安装..."
                fi
            else
                warn "内存过低($(mem_total_mb)MB),跳过 apk 直接使用二进制安装..."
            fi
            if [ -z "$APK_OK" ]; then
                install_singbox_binary || {
                    err "sing-box 二进制安装失败"
                    exit 1
                }
            fi
            ;;
...
action_update() {
    info "开始更新 sing-box..."
    if [ "$OS" = "alpine" ]; then
        apk_update_retry >/dev/null 2>&1 || warn "apk update 失败"
        apk add --upgrade --repository=http://dl-cdn.alpinelinux.org/alpine/edge/community sing-box >/dev/null 2>&1 || {
            warn "apk 更新失败，改用二进制方式更新"
            install_singbox_binary || { err "更新失败"; return 1; }
        }
    else
        bash <(curl -fsSL https://sing-box.app/install.sh)
    fi
...
install_singbox_binary() {
    ...
    ver=$(curl -fsSL --max-time 30 "https://api.github.com/repos/SagerNet/sing-box/releases/latest" 2>/dev/null \
        | sed -n 's/.*"tag_name":[[:space:]]*"v\([^"]*\)".*/\1/p' | head -1)
    [ -z "$ver" ] && err "无法获取 sing-box 版本号" && exit 1
    url="https://github.com/SagerNet/sing-box/releases/download/v${ver}/sing-box-${ver}-linux-${arch}.tar.gz"
    sb_tmp="/usr/local/.sb-install-$$"
    rm -rf /var/cache/apk/* 2>/dev/null || true
    mkdir -p "$sb_tmp"
    info "下载并解压 sing-box ${ver} (流式, 不经过 apk/tmpfs)..."
    curl -fsSL --max-time 600 "$url" | tar -xz -C "$sb_tmp" || { err "下载/解压失败"; rm -rf "$sb_tmp"; exit 1; }
    bin=$(find "$sb_tmp" -type f -name sing-box | head -1)
    [ -z "$bin" ] && { err "未找到 sing-box 二进制"; rm -rf "$sb_tmp"; exit 1; }
    install -m 755 "$bin" /usr/local/bin/sing-box || { err "安装失败"; rm -rf "$sb_tmp"; exit 1; }
    rm -rf "$sb_tmp"
    sync
    [ -x /usr/local/bin/sing-box ] && [ ! -e /usr/bin/sing-box ] && ln -s /usr/local/bin/sing-box /usr/bin/sing-box
}
case "$OS" in
    alpine)
        APK_OK=""
        if command -v apk >/dev/null 2>&1 && [ "$(awk '/MemTotal/{printf "%d", $2/1024}' /proc/meminfo 2>/dev/null)" -ge 96 ]; then
            if apk update >/dev/null 2>&1 && apk add --no-cache \
                --repository=http://dl-cdn.alpinelinux.org/alpine/edge/community \
                sing-box >/dev/null 2>&1; then
                APK_OK=1
                info "sing-box 已通过 apk 安装"
            else
                warn "apk 安装失败,改用官方二进制安装..."
            fi
        else
            warn "内存过低,跳过 apk 直接使用二进制安装..."
        fi
        if [ -z "$APK_OK" ]; then
            install_singbox_binary
        fi
